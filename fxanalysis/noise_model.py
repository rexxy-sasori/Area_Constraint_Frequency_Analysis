...
# only jdht has random weights
def sim_fx_dp_noise_model(noise_level, Bx=B, Bw=B, Ba=B, model_coeff_generator=freq_transform.dht_coeff_jitter):
    ...
    signal_with_noise, _ = generator.get() # get signal with noise

    configurations.noise.init_args.top = 0
    configurations.noise.init_args.steady_state = 0
    generator = signal_generator.InputSignalGenerator(configurations.signal, configurations.noise)
    signal_without_noise, _ = generator.get() # get signal without noise
    ...
    for i in range(Nd):
        ...
        qresult_signal_with_noise[i, :, :] = qsignal_with_noise[i, :, :].dot(coeff_jdht[i, :, :, :].T)
