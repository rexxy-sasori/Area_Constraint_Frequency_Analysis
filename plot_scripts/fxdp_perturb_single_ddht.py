...
@njit
def real_dp_numba(seq, dp_num, dp_dim, coeff, qps=False, bps=0, cps=0):
    result = np.zeros(dp_num)
    for dpidx in range(dp_num):
        total = 0
        for dimidx in range(dp_dim):
            add = coeff[dpidx, dimidx] * seq[dimidx]
            total = total + add
            total = unisign_quant_numba(total, bps, cps, qps)
...
for nl in noise_levels:
    ...
    for b in range(3, 4):
        ...
        #result_max = get_max(result_signal_without_noise)
        result_max = 4 * result_signal_with_noise.std()
        ...
        start = datetime.now()
        for i in range(Nd):
            qresult_signal_with_noise[i, :, :] = real_dp_numba(
        ...
        acc_quant_at_output = N * get_real_quantization_noise_var(deltay) if acc_quant else 0
...
for idx, nl in enumerate(noise_levels):
    ...
    ax.plot(bs, SIMDS[idx], marker='s', color=color[idx], markersize=12,linestyle='dashed')
...
#plt.ylim([10, 26])
plt.savefig('accumulation_only_ddht.png')
